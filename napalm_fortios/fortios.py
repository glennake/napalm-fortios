...
class FortiOSDriver(NetworkDriver):
    ...
    def rollback(self):
        output = self._execute_command_with_vdom(
            "fnsysctl ls -l data2/config", vdom=None
        )
    ...
    def get_facts(self):
        ...
        domain = self._get_command_with_vdom(
            "get system dns | grep domain", vdom="global"
        )["domain"]
    ...
    def get_interfaces_ip(self):
        cmd = self._execute_command_with_vdom(
            "get system interface physical", vdom="global"
        )
        ...
        for line in cmd:
            ...
            if if_name and interface_data[0] == "ip:" and interface_data[1][0] != "0":
                ...
                interface_ip_dictionary[if_name]["ipv4"] = {
                    interface_data[1]: {
                        "prefix_length": sum(
                            bin(int(x)).count("1") for x in interface_data[2].split(".")
                        )
                    }
                }
            if if_name and interface_data[0] == "ipv6:" and interface_data[1][0] != ":":
                try:
                    interface_ip_dictionary[if_name]["ipv6"] = {
                        interface_data[1].split("/")[0]: {
                            "prefix_length": interface_data[1].split("/")[1]
                        }
                    }
                except:
                    interface_ip_dictionary[if_name] = {
    ...
    def get_firewall_policies(self):
        ...
        for key in policy:
            ...
            logtraffic = (
                policy[key]["logtraffic"] if "logtraffic" in policy[key] else False
            )
    ...
    def get_bgp_neighbors(self):
        ...
        command_received = (
            "get router info bgp neighbors {} received-routes | grep prefixes "
        )
        peers = dict()
        ...
        neighbors = {
            n.split()[0]: n.split()[1:] for n in bgp_sum if re.match(re_neigh, n)
        }
        ...
        for neighbor, parameters in neighbors.items():
            ...
            if not neighbor_dict:
                ...
                neighbor_dict["is_enabled"] = (
                    neigh_conf.get_param("shutdown") != "enable" or False
                )
            ...
            detail_output = [
                x.lower()
                for x in self._execute_command_with_vdom(
            ...
            for family in families:
                ...
                received = self._execute_command_with_vdom(
                    command_received.format(neighbor)
                )[0].split()
                if len(received) > 0:
    ...
    def get_interfaces_counters(self):
        ...
        for line in cmd:
            ...
            if (data[0] == "" or data[0] == " ") and len(data) == 1:
                ...
                data[1].startswith("RX packets") or data[1].startswith("TX packets")
            ) and if_name:
                if_data = data[1].split(" ")
                direction = if_data[0].lower()
                interface_counters[if_name][direction + "_unicast_packets"] = int(
                    if_data[1].split(":")[1]
                )
    ...
    def get_environment(self):
        def get_cpu(cpu_lines):
            ...
            for l in cpu_lines:
                m = re.search(
                    "(.+?) states: (.+?)% user (.+?)% system (.+?)% nice (.+?)% idle", l
                )
                cpuname = m.group(1)
                idle = m.group(5)
        ...
        def get_memory(memory_line):
            total, used = (
                int(memory_line[1]) >> 20,
                int(memory_line[2]) >> 20,
            )  # byte to MB
        ...
        sensors_block = self._execute_command_with_vdom(
            "execute sensor list", vdom="global"
        )
        ...
        for line in sensors_block:
            ...
            if m:
                if "TMP" in m.group(2).strip():
                    ...
                    temperatures[sensor_name] = dict(
                        temperature=temp_value, is_alert=False, is_critical=False
                    )
                elif "FAN" in m.group(2):
                    fans[m.group(2) + " - " + m.group(3).strip()] = dict(status=True)
            else:
                m = re.search(
                    "([0-9]+?)\s(.+?)\s+alarm=([0-9]+)\s+value=([0-9\.]+)\s+threshold_status=([0-9]+)",
                    line,
                )
                if m:
                    if "TMP" in m.group(2) or "Temp" in m.group(2):
                        ...
                        fans[m.group(2).strip()] = dict(
                            status=(not bool(int(m.group(3))))
                        )
                    elif m.group(2)[0] == "+":
                        powers[m.group(2).strip()] = dict(
                            status=(not bool(int(m.group(3)))),
                            capacity=-1.0,
                            output=-1.0,
                        )
        ...
        out["cpu"] = get_cpu(
            [
                x
                for x in self._execute_command_with_vdom(
                    "get system performance status | grep CPU", vdom="global"
                )[1:]
                if x
            ]
        )
        ...
        # memory
        memory_block = self._execute_command_with_vdom(
            "diag hard sys mem | grep Mem", vdom="global"
        )
